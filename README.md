...
#### v2 与公开 Computer Use 实践的时间线说明

本项目 v2 在 2026 年 5 月之前，已经实现了一批与后续公开 Computer Use 实践方向相同或相近的能力。这里不宣称这些能力全部由本项目首创，也不把所有能力都归为 Anthropic 官方提出；只是说明：在 v3 之前，项目已经围绕 Windows 本地 Computer Use 场景做了不少工程化实现。

#### v2 与 Anthropic 后续 Computer Use 实践的时间线说明

Anthropic 后续 Computer Use / Browser Use 最佳实践中，重点强调了截图分辨率、坐标缩放、display 尺寸对齐、text-before-image、click issue 诊断、thinking、advisor 等工程实践。

本项目 v2 在 2026 年 5 月之前，已经独立实现了其中一部分相同或相近的工程能力；v3 则在 v2 基础上继续补齐固定分辨率、max_api_fit、半官方模式、双模型顾问、compact、RegionFocus、可视化反馈等能力。

这里不宣称所有能力均为本项目首创，也不宣称 v2 完整覆盖 Anthropic 后续方案；本节只用于说明：v2 已经提前踩中了若干后来被官方重点强调的 Computer Use 工程方向。

| Anthropic 后续公开实践 / 文档重点 | 本项目 v2 是否已有对应实现 | v2 对应情况 | v3 进一步变化 |
|---|---|---|---|
| 应用侧必须实际执行 Computer Use 动作，Claude 只返回工具调用请求 | 已实现 | v2 已有 Windows 本地执行器，负责截图、鼠标、键盘、窗口操作等动作执行 | v3 继续强化为更完整的 Windows 本地 Computer Use 框架 |
| `display_width_px` / `display_height_px` 应与实际截图尺寸一致 | 已实现相近能力 | v2 已有 display 尺寸与截图尺寸对齐、坐标映射相关实现 | v3 在固定分辨率策略下进一步稳定坐标映射 |
| 高分辨率截图需要预处理，避免 2K / 4K 原图导致缩放和点击偏移 | 已实现相近能力 | v2 已支持比例缩放 + 坐标还原 | v3 默认推荐固定 `1280x720`，并支持 `1920x1080` / `max_api_fit` / `scale` |
| 推荐从 `1280x720` 等稳定分辨率开始 | v2 未作为默认主策略 | v2 主要依赖比例缩放，不是固定 720p 作为主推荐 | v3 默认推荐 `1280x720`，作为最稳定通用配置 |
| `max API fit`：按模型图像预算计算合适截图尺寸 | v2 未实现 | v2 没有 max_api_fit 策略 | v3 新增 `max_api_fit`，用于在 API 限制内尽量提高截图细节 |
| 坐标需要从模型截图坐标映射回真实屏幕坐标 | 已实现 | v2 已有缩放后坐标还原逻辑 | v3 在固定分辨率和 max_api_fit 下继续强化坐标转换稳定性 |
| text instruction 应放在 image 之前，减少模型误解 | 已实现 | v2 构造消息时已经采用任务文本在截图前的顺序 | v3 保留并继续作为默认消息构造方式 |
| 使用 Computer Use 工具协议字段 | 已实现部分适配 | v2 已开始适配 Anthropic Computer Use 工具协议字段 | v3 明确增强新版 Computer Use 工作流，半官方 / 官方模式继续强化 |
| click issue 诊断应关注 display 尺寸不匹配、截图缩放、content ordering 等因素 | 已实现部分工程诊断 | v2 已有截图变化检测、replay 复盘、本机自检等能力 | v3.2.2 增强逐动作微验证，并加入 RegionFocus 和更明确失败提示 |
| dense UI / 局部区域可通过放大方式辅助重新定位 | v2 未系统实现 | v2 主要依赖原截图和变化检测 | v3.2.2 新增 RegionFocus 放大重定位 |
| thinking / effort 会影响复杂 Computer Use 任务表现 | v2 支持较弱 | v2 主要是简单 thinking 开关或较少配置 | v3 新增 thinking 强度 / effort 配置 |
| advisor：执行模型遇到困难时咨询更强顾问模型 | v2 未实现 | v2 没有双模型顾问策略 | v3 新增双模型顾问，主模型失败时可切换顾问模型修正 |
| 长任务中需要处理上下文压力和历史膨胀 | v2 未系统实现 compact | v2 长任务主要依赖普通历史管理 | v3 新增长对话 compact 支持 |
| 调试时应能回看模型看到了什么、执行了什么、坐标是否映射正确 | 已实现相近能力 | v2 已有 replay.jsonl + replay.html、本机自检 | v3 保留 replay，并通过 HUD / 可视化反馈进一步增强观察体验 |
| 对高风险动作应加入确认或安全保护 | v2 已有项目侧安全阀 | v2 已有越界拦截、前台窗口安全检查、遮挡区域检测 | v3 继续保留，并把部分失败信号接入顾问触发逻辑 |
