...
##### 与 Anthropic 官方 Computer Use 文档较明确对应的能力

| 能力方向 | 本项目 v2 情况 | 说明 |
|---|---|---|
| Computer Use 执行器 | 已实现 Windows 本地截图、鼠标、键盘执行器 | Anthropic 官方文档要求应用侧实际执行截图、鼠标、键盘动作，Claude 只返回工具调用请求 |
| `display_width_px` / `display_height_px` 对齐 | 已有相关实现 | 用于让模型看到的截图尺寸、工具声明尺寸和坐标映射尽量保持一致 |
| 截图尺寸与坐标映射 | 已支持比例缩放 + 坐标还原 | v3 后进一步升级为固定分辨率 `1280x720`，减少高分屏缩放带来的坐标偏移 |
| Computer Use 工具协议字段 | 已有相关实现 | v2 已开始适配 Anthropic Computer Use 工具协议；v3 继续增强 `computer_20251124` 等新版协议支持 |
| 截图、鼠标、键盘基础动作 | 已支持 | 对应 Computer Use 的核心能力：截图、点击、拖拽、键盘输入等 |
| 动作后确认 / 截图复查 | 已有截图变化检测 | 官方文档建议通过截图检查步骤结果；本项目 v2 已做了工程化的变化检测 |

##### 本项目 v2 自主实现的工程安全阀

下面这些能力更偏 Windows 本地执行器的工程增强，不是 Anthropic 官方文档逐项列出的功能。
...
因此，v3 不是从零开始追随官方实践，而是在 v2 已有桌面执行器、协议适配、截图坐标处理和工程安全阀的基础上，继续补齐和强化了固定分辨率、半官方模式、双模型顾问、compact 长对话压缩、thinking 强度配置、模型列表拉取、MCP Server、内置 Skills 和可视化反馈层等能力。

##### 小结

- v2 已经实现了一批与 Anthropic Computer Use 官方文档方向较明确对应的能力，例如应用侧执行器、`display_width_px` / `display_height_px`、截图与坐标映射、Computer Use 工具协议字段、基础鼠标键盘动作等。
- v2 同时也包含一批项目侧工程安全阀，例如越界拦截、前台窗口校验、遮挡检测、DOM 辅助、窗口激活、replay 复盘和本机自检。
- 本节只用于说明项目演进时间线和工程设计思路，不宣称所有能力均为本项目首创，也不把项目侧安全阀全部归为 Anthropic 官方能力。
---
